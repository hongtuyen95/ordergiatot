        ...
        if ($v->parent_id == $parent) {
            ...
            echo "<tr>
                    <td class='detail_list' style='text-align:center;'>
                         <input type='checkbox' name='checkone[]' id='checkone' value='".$v->id."'  onclick='DoCheckOne('frmCatNew')' />
                    </td>
                     <td>
                        <input type='number' value='".@$v->sort."' data-item='".$v->id."' onchange='cat_sort($(this))' style='width: 45px; padding: 2px;border:1px solid #ddd'/>
                    </td>
    ...
    if(is_array($data)&&$id!=null){
        ...
        foreach($data as $row){
            if($row->id==$id){
                ...
                echo '<i class="fa fa-angle-double-right"></i><a href="'.base_url(@$row->alias.'-'.$arr['cat'].$row->id).'">  '.@$row->name.' </a>';
            }
        }
    }
}
