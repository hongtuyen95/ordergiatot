...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><a href="<?= base_url('vnadmin/aitribute/listOption')?>">Danh sách kích thước</a></li>
...
<section class="content">
    ...
    <div class="box">
        <div class="box-header">
            <a href="<?= base_url('vnadmin/aitribute/addoption')?>" class="btn btn-success btn-sm">
        ...
        <!-- /.box-header -->
        <div class="box-body">
			<form name="formbk" method="post" action="<?=base_url('vnadmin/attribute/deletes_option_category')?>">
                <table id="example" class="table table-bordered table-striped">
					<thead>
                        <tr>
							...
							<th width="">kích thước</th>
							<th width="20%" class="no-sort">size</th>
					 ...
					 <?php if (isset($list)) {
						...
						<tr>
							...
							<td><input type="number" onchange="update_sort($(this))" value="<?= @$v->sort ?>"
								data-placement='product_size' data-item='<?= @$v->id; ?>' style="width: 55px"></td>
							<td><?= @$v->name ?></td>
							<td><?= @$v->size ?></td>
							<td class="hidden">
                                <?= @$v->note ?>
							...
							<td class="text-center">
								<a class="btn btn-xs btn-default"
								   href="<?= base_url('vnadmin/attribute/editoption/' . $v->id) ?>"><i
										class="fa fa-pencil"></i> </a>
								<a class="btn btn-xs btn-danger"
								   href="<?= base_url('vnadmin/attribute/deleteopioncategory/' . $v->id) ?>" title="Xóa"
