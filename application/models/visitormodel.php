...
class VisitorModel extends CI_Model {
    ...
    function Delete_site_log() {
        $results = array();
        $query = $this->db->query('DELETE
            FROM site_log    
            WHERE DATE(access_date) < CURDATE() - INTERVAL DAYOFWEEK(CURDATE())-1 DAY');
	...
	function get_site_data_for_yesterday() {
        $results = array();
        $query = $this->db->query('SELECT SUM(no_of_visits) as visits
            FROM ' . $this->site_log . '	
			WHERE DATE(access_date) = CURDATE() - INTERVAL 1 DAY
            LIMIT 1');
    ...
    function get_site_data_for_last_week() {
        ...
        $query = $this->db->query('SELECT SUM(no_of_visits) as visits
            FROM ' . $this->site_log . '
            WHERE DATE(access_date) >= CURDATE() - INTERVAL DAYOFWEEK(CURDATE())+7 DAY
	...
	function get_site_data_for_total() {
        $results = array();
        $query = $this->db->query('SELECT SUM(no_of_visits) as visits
            FROM ' . $this->site_log . '
            WHERE no_of_visits = 1
            LIMIT 1');
        if ($query->num_rows() == 1) {
            $row = $query->row();
            $results['visits'] = $row->visits;
