    ...
    public  function order_payment(){
        ...
        if(isset($_POST['sendcart'])){
            ...
            for($i=0; $i< sizeof($_POST['item_id']);$i++){
                ...
                $this->f_shoppingcartmodel->Update_where('product',array('id'=>$temp_item->id),array( 'bought' => $buyted ));
            ...
            if($id){
                ...
                @$province = $this->f_shoppingcartmodel->getFirstRowWhere('province',array('id'=>$this->input->post('province')));
                @$district = $this->f_shoppingcartmodel->getFirstRowWhere('district',array('id'=>$this->input->post('district')));
                @$ward = $this->f_shoppingcartmodel->getFirstRowWhere('ward',array('id'=>$this->input->post('ward')));
                $this->load->library('email', @$config);
                ...
                foreach($_SESSION['cart'] as $key => $tcat){
                    ...
                    if(!empty($tcat['options'])){
                        ...
                        for($i = 0; $i < count($t_opt); $i++){
                            $temp[$i] = $this->f_shoppingcartmodel->get_data('oc_option_value', array('id' => $t_opt[$i]));
...
//                mail(''.$this->site_email.'', "$subject", $body,$headers);
        ...
        $data['shipping'] =  $this->f_shoppingcartmodel->get_data('shipping');
...
//            redirect($_SERVER['HTTP_REFERER']);
    ...
    public function history_transform(){
        $data = array();
        $id = $this->input->get('id');
        ...
        $this->load->view('carts/view_transform',$data);
    }
}