...
class Ajax_pagination{
    function create($total, $number_per_page, $current_page = 1, $class='paginate_button', $classCurrent = 'paginate_active'){
        ...
        if($last_page > 1){
            ...
            // Trang trước
            if($current_page > 1){
                $pagination_system.= "<li><a class=\"first paginate_button\"  page='".$previous_page."' onclick=\"pagenext(".$previous_page.")\">Trước</a></li>"; }
            else{
            ...
            if ($last_page < 7 + ($pagination_stages * 2)){
                ...
                for ($page_counter = 1; $page_counter <= $last_page; $page_counter++){
                    if ($page_counter == $current_page) {
                        ...
                        $pagination_system.= "<li><a class='paginate_button' page=".$page_counter." onclick=\"pagenext(".$page_counter.")\">$page_counter</a></li>";
                    }
                }
                $pagination_system .='</span>';
                ...
                if($current_page < 1 + ($pagination_stages * 2)){
                    ...
                    $pagination_system.= "<li><a class=\"paginate_button\" page=".$last_paged." onclick=\"pagenext(".$last_paged.")\">$last_paged</a></li>";
                    $pagination_system.= "<li><a class=\"paginate_button\"  page=".$last_page." onclick=\"pagenext(".$last_paged.")\">$last_page</a></li>";
                    $pagination_system .='</span>';
                }elseif($last_page-($pagination_stages*2) > $current_page && $current_page > ($pagination_stages*2)){
                    $pagination_system .='<span>';
                    $pagination_system .= "<li><a class=\"paginate_button\"  page=\"1\" onclick=\"pagenext(1)\">1</a></li>";
                    $pagination_system .= "<li><a class=\"paginate_button\"  page=\"2\" onclick=\"pagenext(2)\">2</a></li>";
                    $pagination_system .= "<li><a class=\"paginate_button disabled\" >...</a></li>";
            ...
            if ($current_page < $page_counter - 1) {
                $pagination_system.= "<li><a class=\"next paginate_button\"  page=".$next_page." onclick=\"pagenext(".$next_page.")\">Tiếp theo</a></li>";
            }else{
